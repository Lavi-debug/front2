...
//     const storedLicense = localStorage.getItem('Plicense');
//     if (storedLicense) {
...
//       setErrorMessage('');
//       setIsLoggedIn(true);
//       localStorage.setItem('Plicense', license);
//     } else {
//       setErrorMessage('Invalid details. Please check the contact and bus number.');
//       setFilteredStudents([]);
//       setShowParents(false);
...
//     // Clear localStorage
//     localStorage.removeItem('Plicense');
//   };
...
//         </div>
//       ) : (
//         <Parents license={license} contact={contact} onLogout={handleLogout} />
//       )}
//     </div>
//   );
// };
...
const Loginparents = () => {
  ...
  useEffect(() => {
    const storedLicense = localStorage.getItem('Plicense');
    const storedContact = localStorage.getItem('Pcontact');
    if (storedLicense && storedContact) {
      setLicense(storedLicense);
      setContact(storedContact);
      setIsLoggedIn(true);
      setShowParents(true); // Show Parents component directly
    }
  }, []);
  ...
  const handleLogin = (e) => {
    ...
    if (foundStudents.length > 0) {
      ...
      localStorage.setItem('Plicense', license);
      localStorage.setItem('Pcontact', contact);
    } else {
      setErrorMessage('Invalid details. Please check the contact and bus number.');
      setFilteredStudents([]);
      setShowParents(false);
  ...
  const handleLogout = () => {
    ...
    // Clear localStorage
    localStorage.removeItem('Plicense');
    localStorage.removeItem('Pcontact');
  };
